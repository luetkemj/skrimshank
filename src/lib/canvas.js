...
import { getEntitiesAtPos, getEntity } from "./ecsHelpers";
...
const initPixiContainerRow = ({ container, row }) => {
  ...
  _.times(width, (i) => {
    ...
    sprite.on("click", () => {
      const ents = getEntitiesAtPos({ x: i, y: row });
      ents.forEach((e) => {
        if (e.brain) {
          e.goals = e.brain.goalIds.map((gid) => getEntity(gid));
        }
      });

      console.log({ sprite, ents });
    });
